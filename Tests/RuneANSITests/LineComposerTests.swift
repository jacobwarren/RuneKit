...
struct LineComposerTests {
    ...
    func splitPreserves() {
        ...
        let lText = ANSITokenizer().tokenize(l).compactMap { if case let .text(t) = $0 { t } else { nil } }.joined()
        let rText = ANSITokenizer().tokenize(r).compactMap { if case let .text(t) = $0 { t } else { nil } }.joined()
        #expect(lText == "AB")
    ...
    func wrapRoundTrip() {
        ...
        let toks = ANSITokenizer().tokenize(joined).compactMap { if case let .text(t) = $0 { t } else { nil } }.joined()
        let orig = ANSITokenizer().tokenize(s).compactMap { if case let .text(t) = $0 { t } else { nil } }.joined()
        #expect(toks == orig)
    }
}
...
func truncatePreventsBleed() {
    ...
    let postResetSGRCount = rendered.drop { $0 != .sgr([0]) }
        .count(where: { if case .sgr = $0 { true } else { false } })
